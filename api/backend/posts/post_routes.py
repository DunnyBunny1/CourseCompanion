...
def get_all_posts():
    """
    "GET /posts?courseId=3001&sectionId=1 HTTP/1.1"
    """
...
def create_post():
    current_app.logger.info('PUT /posts endpoint')
    try:
        post_data = request.json
        ...
        query = '''
            INSERT INTO posts (title, content, isAnnouncement, authorId, courseId, sectionId)
            VALUES (%s, %s, %s, %s, %s, %s)
        '''

        values = (title, content, isAnnouncement, authorId, courseId, sectionId)

        current_app.logger.info(query, values)
        cursor = db.get_db().cursor()
        cursor.execute(query, values)
        db.get_db().commit()
        the_response = make_response(jsonify({"message": f"Post created successfully"}))
        the_response.status_code = 200
        the_response.mimetype = 'application/json'
        
    except: 
        current_app.logger.error("error occured", exc_info=True)
        the_response = "error"
    return the_response
